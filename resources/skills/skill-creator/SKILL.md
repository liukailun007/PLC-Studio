...
## PLC Studio workflow (READ FIRST — overrides packaging / install steps below)

You are running inside PLC Studio. Skills live in a managed registry that PLC
...
**To create a new skill, write it straight into PLC's managed skills directory:**

1. Resolve the directory once by running `echo "$CHERRY_STUDIO_SKILLS_DIR"` in Bash.
   That folder is PLC's managed skill library.
2. Create `$CHERRY_STUDIO_SKILLS_DIR/<skill-folder-name>/` and write `SKILL.md` plus any
   ...
   tools.
3. That's it. PLC's skill sync detects the new directory, registers it in the catalog,
