...
PLC gives you two built-in tools for skills — use them, and do NOT shell out to
...
- **`install_skill(install_source)`** — install ONE skill into PLC's managed
  library and enable it for this agent. PLC clones the repo, installs just that one skill,
...
3. That you can install it for them into PLC
...
I can install it into PLC's skill library for you — want me to go ahead?
...
outside PLC's library. `install_skill` installs **only that one skill** into PLC's
...
me to "create a skill for <task>" and I'll write one into PLC's skills directory.
```
