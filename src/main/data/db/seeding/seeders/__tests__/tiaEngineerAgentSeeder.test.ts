...
describe('TiaEngineerAgentSeeder', () => {
  ...
  it('creates the TIA engineer agent with the full instructions and a system session', () => {
    ...
    // ProjectContext discipline ships with the agent so every fresh install gets it.
    expect(agent.instructions).toContain('tia-project-scanner')
    expect(agent.instructions).toContain('省 token')
