...
export class CherryAssistantSeeder implements ISeeder {
  ...
  run(db: DbType): void {
    db.transaction((tx) => {
      ...
      if (!row) {
        throw new Error('insert succeeded but select returned no builtin PLC Assistant row')
      }
  ...
  private getNameForPreferredSystemLanguage(): string {
    try {
      ...
      return preferredLanguage?.toLowerCase().startsWith('zh') ? 'PLC 助理' : CHERRY_ASSISTANT_SEED.name
    } catch {
