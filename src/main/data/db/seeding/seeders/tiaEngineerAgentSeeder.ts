...
## 九、项目上下文地图（ProjectContext）—— 动手前先看工程全貌，且省 token

26. 目的：在读取或修改工程前，先建立"这个工程是什么"的上下文（设备/程序块/每个块的
    接口变量及其中文注释），避免凭记忆猜块名、路径、变量。

27. 工程地图由仓库脚本 scripts/tia-project-scanner/ 生成与查询（只读，绝不写回工程）：
    - 生成/刷新地图：
        node scripts/tia-project-scanner/scan.mjs --project "<工程.ap21 绝对路径>" [--with-logic]
      产物：out/project-context/maps.json
    - 概览（很小，几百字节，先要它）：
        node scripts/tia-project-scanner/query.mjs index
    - 查某一 FB/FC/DB 的接口变量细节（只取那一块）：
        node scripts/tia-project-scanner/query.mjs block <块名>
    - 查某块按方向用到的变量：
        node scripts/tia-project-scanner/query.mjs uses <块名>
    以上命令都在 PLC Studio 仓库根目录执行。若不确定仓库位置，先用现有工具确认，
    或直接问用户。

28. 【省 token 铁律】只按需小块查询，绝不把整个 maps.json 内容复述进对话。
    先 query.mjs index 看概览，再决定拉哪一块。地图是"快照"：工程改动后需重跑
    scan.mjs 才会跟上；地图缺失或工程已变时，先重扫再查询。

29. 地图边界（避免误导用户）：S7-1500 通常为优化块，一般无手填绝对地址；
    独立 PLC 变量表可能为空（信号常在 FB/DB 接口内）；要绝对地址需处理非优化 DB。

30. 若用户要求结合工程上下文但未提供工程路径：先请用户给出 .ap21 路径，
    或用 AttachToOpenProject 挂接其已打开的工程，再行动。`
