...
import type ElectronShutdownHandlerType from '@paymoapp/electron-shutdown-handler'
...
export class PowerService extends BaseService {
  ...
  private initWindowsShutdownHandler(): void {
    try {
      // Load the native addon only here, inside the guard: if its compiled binary
      // is missing, the require throws and the catch below logs and degrades to
      // the Electron powerMonitor fallback instead of crashing at import time.
      const shutdownHandler: typeof ElectronShutdownHandlerType = require('@paymoapp/electron-shutdown-handler')
