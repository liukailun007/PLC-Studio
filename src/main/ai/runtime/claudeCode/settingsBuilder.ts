...
/**
 * Build a lightweight environment snapshot (~200 tokens) for PLC Assistant.
...
export async function buildClaudeCodeSessionSettings(
  ...
  // Assistant diagnostics there. Local PLC Assistant sessions keep the full MCP.
...
export async function buildSystemPrompt(
  ...
  if (builtinRole && !instructions) {
    ...
    if (definition?.instructions) {
      ...
      logger.error('Builtin PLC Assistant definition missing; using minimal fallback instructions')
...
export function buildMcpServers(
  ...
  // 5. Assistant — navigate + diagnose tools (local PLC Assistant sessions only)
  if (assistantMcpEnabled) {
    ...
    logger.debug('PLC Assistant: injected assistant MCP server', {
