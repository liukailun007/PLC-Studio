...
/**
 * Repair a JSON-Schema-shaped value so every `type: "array"` node declares an
 * `items` entry. Strict backends (Gemini, DeepSeek, VS Code) reject arrays whose
 * `items` is absent, yet many MCP servers emit exactly that for untyped array
 * parameters (a bare `JsonArray` serializes as `{ type: "array" }`). An empty
 * schema (`{}`) means "any item", which matches an untyped array's intent.
 */
function ensureArrayItems(node: unknown): void {
  if (Array.isArray(node)) {
    for (const entry of node) ensureArrayItems(entry)
    return
  }
  ...
  for (const value of Object.values(schema)) ensureArrayItems(value)
}
