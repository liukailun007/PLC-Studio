      ...
      try {
        ...
        const initTransport = async (
          ...
          if (isBuiltinMcpServer(server) && server.name !== BuiltinMcpServerNames.mcpAutoInstall) {
            ...
            // Resolve `cherry-resource://` markers (bundled binaries, e.g. the TIA Portal
            // Openness MCP server) to the current app resources location at spawn time —
            // the stored command stays location-independent across installs/moves.
            let cmd = resolveBundledMcpCommand(server.command)
