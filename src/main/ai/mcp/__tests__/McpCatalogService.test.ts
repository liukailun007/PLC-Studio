...
describe('McpCatalogService', () => {
  ...
  it('repairs array-typed parameters that omit items (strict backends reject them)', async () => {
    getById.mockReturnValue(server())
    listTools.mockResolvedValue({
      tools: [
        {
          name: 'invokeObject',
          description: 'invoke desc',
          inputSchema: {
            type: 'object',
            properties: {
              args: { description: 'JSON array of args', type: 'array', default: null },
              nested: { type: 'object', properties: { values: { type: 'array' } } }
            },
            required: ['args']
          }
        }
    ...
    const [tool] = cacheStore.get('mcp.tools.server-1') as Array<{
      inputSchema: { properties: Record<string, { items?: unknown; properties?: Record<string, { items?: unknown }> }> }
    }>
    expect(tool.inputSchema.properties.args.items).toEqual({})
    expect(tool.inputSchema.properties.nested.properties?.values.items).toEqual({})
  })
