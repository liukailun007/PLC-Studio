...
type OnboardingStep = 'welcome' | 'openai-hub' | 'provider' | 'select-model'
type OnboardingCompletionStatus = Exclude<OnboardingProviderSetupStatus, 'pending'>
type PrivacyChoiceAction = () => void | Promise<void>

const OPENAI_HUB_REGISTER_URL = 'https://api.openai-hub.net/register?aff=kwxm'
...
export default function OnboardingPage() {
  ...
  const { addApiKey, updateProvider } = useProvider('openai-hub')
  const { syncProviderModels } = useProviderModelSync('openai-hub')
  ...
  const handleOpenAIHubConnect = useCallback(async () => {
    ...
    try {
      ...
      const models = await syncProviderModels()
      if (!models.some((model) => model.isEnabled)) {
        toast.error(t('onboarding.provider_setup.missing_model'))
        setStep('provider')
        return
      }
      toast.success(t('onboarding.openai_hub.connected'))
      setStep('select-model')
    } catch {
      toast.error(t('onboarding.openai_hub.connect_failed'))
    } finally {
      setIsConnecting(false)
    }
  }, [addApiKey, apiKey, syncProviderModels, t, updateProvider])
          ...
          <div className="relative min-h-0 flex-1 overflow-hidden">
            {step === 'welcome' && (
              <div className="flex h-full w-full items-center justify-center px-6 pb-20">
                <div className="flex w-full max-w-[420px] flex-col items-center">
                  <img src={AppLogo} alt="PLC Studio" className="size-16 rounded-xl" />
                  ...
                  <div className="mt-8 flex w-full flex-col gap-3">
                    <Button
                      ...
                      onClick={() => void runAfterPrivacyChoice(() => setStep('openai-hub'))}>
                      <KeyRound size={16} />
                      {t('onboarding.welcome.login_openai_hub')}
                    </Button>
                    <Button
                      type="button"
                      variant="outline"
            ...
            {step === 'openai-hub' && (
              <div className="flex h-full min-h-0 w-full flex-col">
                <OnboardingHeader title={t('onboarding.openai_hub.title')} onBack={() => setStep('welcome')} padded />
                <div className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto px-6 py-8">
                  <div className="flex w-full max-w-[420px] flex-col items-center">
                    <div className="flex w-full flex-col gap-2 text-center">
                      <h2 className="m-0 font-semibold text-foreground text-lg">{t('onboarding.openai_hub.title')}</h2>
                      <p className="m-0 text-foreground-secondary text-sm">{t('onboarding.openai_hub.subtitle')}</p>
                    </div>
                    <div className="mt-8 flex w-full flex-col gap-3">
                      <Input
                        type="password"
                        ...
                        spellCheck={false}
                        value={apiKey}
                        placeholder={t('onboarding.openai_hub.api_key_placeholder')}
                        aria-label={t('onboarding.openai_hub.api_key_placeholder')}
                      ...
                      <Button
                        ...
                        disabled={!apiKey.trim() || isUpdatingPrivacy}
                        onClick={() => void handleOpenAIHubConnect()}>
                        {t('onboarding.openai_hub.connect')}
                      </Button>
                    </div>
                    <a
                      className="mt-4 text-primary text-xs hover:underline"
                      href={OPENAI_HUB_REGISTER_URL}
                      target="_blank"
                      rel="noreferrer">
                      {t('onboarding.openai_hub.get_api_key')}
                    </a>
                  </div>
                </div>
              </div>
