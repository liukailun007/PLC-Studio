...
import { APP_NAME, MODIFIER_NAME, SOURCE_CODE_URL } from '@shared/utils/constants'
import { debounce } from 'es-toolkit/compat'
import { Bug, Github, Rss, Scale } from 'lucide-react'
    ...
    <SettingsContentColumn theme={theme}>
      <SettingGroup theme={theme}>
        <SettingTitle className="gap-2">
          ...
          <button
            type="button"
            onClick={() => onOpenWebsite(SOURCE_CODE_URL)}
      ...
      <SettingGroup theme={theme}>
        <SettingTitle className="gap-2">
          ...
          <span className="font-semibold text-[15px]">{t('settings.about.license.title')}</span>
        </SettingTitle>
        <Divider className="my-1.5" />
        <div className="flex flex-col gap-2 py-1 text-sm text-foreground-secondary">
          <p>{t('settings.about.license.modifiedBy', { modifier: MODIFIER_NAME })}</p>
          <p>{t('settings.about.license.basedOn')}</p>
          <p>{t('settings.about.license.agpl')}</p>
        </div>
      </SettingGroup>
      ...
      <SettingGroup theme={theme}>
        <AboutActionRow
          ...
          actionLabel={t('settings.about.sourceCode.button')}
          onAction={() => onOpenWebsite(SOURCE_CODE_URL)}
        />
        <Divider className="my-3" />
        <AboutActionRow
