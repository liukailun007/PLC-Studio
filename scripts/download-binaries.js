...
// Optional GitHub release proxy: set BINARIES_MIRROR (e.g. https://ghfast.top/ )
// when github.com is unreachable (e.g. mainland China). The prefix is prepended
// to every GitHub download URL; the SHA256 check still guards integrity.
const MIRROR_PREFIX = process.env.BINARIES_MIRROR || ''

function miseUrl(file) {
  return `${MIRROR_PREFIX}https://github.com/jdx/mise/releases/download/v${MISE_VERSION}/${file}`
}
function bunUrl(asset) {
  return `${MIRROR_PREFIX}https://github.com/oven-sh/bun/releases/download/bun-v${BUN_VERSION}/${asset}.zip`
}
function uvUrl(asset, ext) {
  return `${MIRROR_PREFIX}https://github.com/astral-sh/uv/releases/download/${UV_VERSION}/${asset}.${ext}`
}
function rgUrl(asset, ext) {
  return `${MIRROR_PREFIX}https://github.com/BurntSushi/ripgrep/releases/download/${RG_VERSION}/ripgrep-${RG_VERSION}-${asset}.${ext}`
}
function mingitUrl(asset) {
  return `${MIRROR_PREFIX}https://github.com/git-for-windows/git/releases/download/v${MINGIT_VERSION}.windows.1/${asset}`
}
